...
def test_usgs_landsat_stac_transform(usgs_landsat_stac):
    ...
    expected_geometry_coordinates = [
        [
            [1087485.0, -313215.0], [904635.0, -356445.0], 
            [860745.0, -172485.0], [1043085.0, -128325.0], 
            [1087485.0, -313215.0]
        ]
    ]
    assert transformed_stac_doc['geometry']['coordinates'] == expected_geometry_coordinates
...
def test_lidar_stac_transform(lidar_stac):
    ...
    expected_geometry_coordinates = [
        [
            [765999.0, 6731999.0], [765999.0, 6729999.0], 
            [767999.0, 6729999.0], [767999.0, 6731999.0], 
            [765999.0, 6731999.0]
        ]
    ]
    assert transformed_stac_doc['geometry']['coordinates'] == expected_geometry_coordinates