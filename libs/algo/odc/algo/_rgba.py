...
def auto_guess_clamp(ds: xr.Dataset):
    ...
    return (0, max(x.data.max()
                   for x in ds.data_vars.values()))
...
def to_rgba_np(r: np.ndarray,
               g: np.ndarray,
               b: np.ndarray,
               nodata: Optional[float],
               clamp: Tuple[float, float]) -> np.ndarray:
...
def to_rgba(ds: xr.Dataset,
    ...
    coords = {name: coord
              for name, coord in red_band.coords.items()}
