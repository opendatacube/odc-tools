    ...
    def measurements(self, input_measurements):
        if self.mask_measurement_name not in input_measurements:
            raise VirtualProductException(
                "required measurement {} not found".format(self.mask_measurement_name)
            )
...
class ApplyMask(Transformation):
    ...
    def __init__(
        self,
        mask_measurement_name,
        apply_to: Optional[Collection[str]] = None,
        preserve_dtype=True,
    ...
    def compute(self, data):
        ...
        if self.erosion > 0:
            from skimage.morphology import binary_erosion, disk  # pylint: disable=C0415
        ...
        if self.dilation > 0:
            from skimage.morphology import (  # pylint: disable=C0415
                binary_dilation,
                disk,
            )
        ...
        def worker(key, value):
            if self.preserve_dtype:
                if "nodata" not in value.attrs:
                    raise VirtualProductException(
                        "measurement {} has no nodata value".format(key)
                    )
