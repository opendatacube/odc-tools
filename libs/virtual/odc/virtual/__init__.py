...
class NameResolver:
    ...
    def construct(self, **recipe) -> VirtualProduct:
        ...
        if kind == "transform":
            ...
            self._assert(
                ...
                "no input for transformation in {}".format(recipe),
            )
        ...
        if kind == "collate":
            self._assert(
                len(recipe["collate"]) > 0,
                "no children for collate in {}".format(recipe),
            )
        ...
        if kind == "juxtapose":
            self._assert(
                len(recipe["juxtapose"]) > 0,
                "no children for juxtapose in {}".format(recipe),
            )
        ...
        if kind == "aggregate":
            ...
            self._assert(
                input_product is not None, "no input for aggregate in {}".format(recipe)
            )
            self._assert(
                group_by is not None, "no group_by for aggregate in {}".format(recipe)
            )
        ...
        if kind == "reproject":
            ...
            self._assert(
                input_product is not None, "no input for reproject in {}".format(recipe)
            )
            self._assert(
                output_crs is not None,
                "no output_crs for reproject in {}".format(recipe),
            )
            self._assert(
                resolution is not None,
                "no resolution for reproject in {}".format(recipe),
            )
        ...
        raise VirtualProductException(
            "could not understand virtual product recipe: {}".format(recipe)
        )
    ...
    def register(self, namespace: str, name: str, callable_obj):
        ...
        if name in self.lookup_table[namespace]:
            raise VirtualProductException(
                "name {} under {} is already registered".format(name, namespace)
            )
