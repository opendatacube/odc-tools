...
class VirtualDatasetBag:
    ...
    def __repr__(self):
        return "<VirtualDatasetBag of {} datacube datasets>".format(
            len(list(self.contained_datasets()))
        )
...
class VirtualDatasetBox:
    ...
    def __repr__(self):
        if not self.load_natively:
            return "<VirtualDatasetBox of shape {}>".format(
                dict(zip(self.dims, self.shape))
            )
...
class VirtualProduct(Mapping):
    ...
    def output_measurements(
        self, product_definitions: Dict[str, DatasetType]
    ...
    def __repr__(self):
        return yaml.dump(
            self._reconstruct(), Dumper=SafeDumper, default_flow_style=False, indent=2
        )
...
class Product(VirtualProduct):
    ...
    def output_measurements(
        self,
        product_definitions: TypeMapping[str, DatasetType],  # type: ignore[override]
...
class Transform(VirtualProduct):
    ...
    def _transformation(self) -> Transformation:
        ...
        try:
            ...
            raise VirtualProductException(
                "transformation {} could not be instantiated".format(cls)
            )

        self._assert(
            isinstance(obj, Transformation),
            "not a transformation object: {}".format(obj),
        )
...
class Juxtapose(VirtualProduct):
    ...
    def output_measurements(
        ...
        for measurements in input_measurement_list:
            ...
            self._assert(
                not common, "common measurements {} between children".format(common)
            )
            ...
            result[measurement] = xarray.concat(
                [
                    xr_reproject(
                        raster[measurement],
                        geobox,
                        resampling=band_settings[measurement]["resampling_method"],
                        no_data=raster[measurement].nodata,
                    )
