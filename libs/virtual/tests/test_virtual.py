...
from odc.virtual import construct_from_yaml, catalog_from_yaml, VirtualProductException
from odc.virtual import DEFAULT_RESOLVER, Transformation
from odc.virtual.impl import Datacube

from odc.virtual.expr import formula_parser, FormulaEvaluator, evaluate_data
from odc.virtual.transformations import fiscal_year
...
def example_product(name):
    ...
    pixelquality = dict(
        name="pixelquality", dtype="int16", nodata=0, units="1", flags_definition=flags
    )
        ...
        products:
            ...
            cloud_free_ls7_nbar:
                ...
                recipe:
                    ...
                    transform: odc.virtual.transformations.ApplyMask
                    mask_measurement_name: pixelquality
                    input:
                      <<: *cloud_mask_recipe
                      input:
...
def test_load_data(cloud_free_nbar, dc, query):
    with mock.patch("odc.virtual.impl.Datacube") as mock_datacube:
...
def test_aggregate(dc, query, catalog):
    ...
    with mock.patch(
        "odc.virtual.impl.Datacube"
    ) as mock_datacube, warnings.catch_warnings():
...
def test_reproject(dc, query, catalog):
    ...
    print(f"*****decipher recipe {type(reproject_utm)}*****")
    ...
    assert data.odc.geobox.crs == CRS("EPSG:32755")
